  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-mist via-white to-mist relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40">
        <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-gradient-to-br from-royal/20 to-transparent rounded-full blur-3xl -translate-x-1/3 -translate-y-1/3" />
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-gradient-to-tl from-gold/10 to-transparent rounded-full blur-3xl translate-x-1/3 translate-y-1/3" />
      </div>
        ...
        >
          ...
          <div className="text-center mb-8">
            <motion.div
              ...
              className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-gold to-amber rounded-2xl mb-6 shadow-lg"
            >
            ...
            <p className="text-carbon text-base">
              Join Printelya and start creating custom products
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-md border border-mist p-8 space-y-6">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white text-graphite">Or create with email</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  {...register('name')}
                  className={`bg-mist border-carbon h-12 ${errors.name ? 'border-velvet' : ''}`}
                />
                {errors.name && (
                  <p className="text-sm text-velvet">{errors.name.message}</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-graphite">Confirm Password</label>
                <div className="relative">
                  <Input
                    type={showConfirmPassword ? 'text' : 'password'}
                    placeholder="Confirm your password"
                    {...register('confirmPassword')}
                    className={`bg-mist border-carbon h-12 pr-12 ${errors.confirmPassword ? 'border-velvet' : ''}`}
                  />
                  <button
                    type="button"
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-graphite hover:text-onyx transition-colors"
                ...
                {errors.confirmPassword && (
                  <p className="text-sm text-velvet">{errors.confirmPassword.message}</p>
                )}
              </div>
            ...
            <div className="text-center pt-4 border-t border-mist">
              <p className="text-graphite text-sm">
                ...
                <Link to="/login" className="text-onyx hover:text-gold font-semibold transition-colors">
                  Sign In
                </Link>
              </p>
            </div>
